...
def _box_edges(box):
    """The four boundary segments of an axis-aligned box."""
    x0, y0, x1, y1 = box
    return [(x0, y0, x1, y0), (x1, y0, x1, y1),
            (x1, y1, x0, y1), (x0, y1, x0, y0)]


def _leader_touch(px, py, segs):
    """A leader-end point that lands on the tagged geometry.

    `segs` is a list of segments (x0,y0,x1,y1). For a spool these are its member
    pipe centrelines (plus fitting points as degenerate segments), so the leader
    end sits ON a pipe rather than off the loose assembly box; for a hanger they
    are its box edges. Returns the point on the nearest segment closest to the
    approach (px, py) -- short leader, touching real geometry.
    """
    if not segs:
        return (px, py)
    best = None
    for s in segs:
        qx, qy = _nearest_on_seg(px, py, s)
...
def _member_segments(el, view):
    """Plan segments of an assembly's members: pipe centrelines, fitting points,
    or member box edges as a fallback -- so a leader can land on real pipe, not
    the loose assembly box. Returns None for non-assemblies."""
    try:
        ids = el.GetMemberIds()
    except Exception:
        return None
    ...
    for mid in ids:
        try:
            m = el.Document.GetElement(mid)
        except Exception:
            m = None
        if m is None:
            continue
        loc = getattr(m, "Location", None)
        cur = getattr(loc, "Curve", None) if loc is not None else None
        ...
        if cur is not None:
            try:
                ...
                pass
        if not added and pt is not None:
            segs.append((pt.X, pt.Y, pt.X, pt.Y))
            added = True
        if not added:
            # Fabrication fittings (elbows, couplings) have no location
            # curve/point; their connectors sit on the centreline at each
            # face, so chords between connector origins lie on the real
            # fitting body -- unlike its loose bounding box.
            origins = []
            try:
                cm = getattr(m, "ConnectorManager", None)
                if cm is None:
                    mep = getattr(m, "MEPModel", None)
                    cm = getattr(mep, "ConnectorManager", None) if mep else None
                if cm is not None:
            ...
            if len(origins) >= 2:
                ...
                a = origins[0]
                segs.append((a.X, a.Y, a.X, a.Y))
                added = True
        if not added:
            bb = m.get_BoundingBox(view)
            if bb is not None:
                segs.extend(_box_edges(
...
def _callout_regions(doc, view):
    ...
    Only callout views that are actually placed on a sheet count: a callout
    that never made it onto a sheet details nothing, so the parent view must
    keep tagging the hangers inside it. (Line-set plans with big un-sheeted
    callouts otherwise lose every hanger to the exclusion and tag zero.)"""
    regions = []
    ref_names = set()
    ...
    for vw in DB.FilteredElementCollector(doc).OfClass(DB.View):
        try:
            if vw.Name not in ref_names:
                continue
            if vw.Id.IntegerValue not in placed_ids:
                continue        # callout not on any sheet -- tag here instead
            cb = vw.CropBox
            tr = cb.Transform
            xs = []
            ys = []
...
def _spot_search(ebox, hx0, hy0, hw, hh, obstacle_grid, other_tags,
    ...
    through another leader. (hw, hh) are the HORIZONTAL half-dimensions; the tag
    is tried horizontal first, and only if no clean horizontal spot exists is the
    vertical (rotated) footprint tried -- so tags stay horizontal unless the
    space is tight. Returns (ln, nx, ny, st, nb, sseg, vertical) or None.
    """
    reach = max_move + 6.0
    local_obs = _grid_near(obstacle_grid,
                           (hx0 - reach, hy0 - reach, hx0 + reach, hy0 + reach))
    for vert in (False, True):
        chw, chh = (hh, hw) if vert else (hw, hh)
        best = None
        for d in _NUDGE_RINGS:
            ...
            for ang in range(0, 360, 10):
                ...
                nb = (nx - chw, ny - chh, nx + chw, ny + chh)
                ...
                if hit:
                    continue
                # New box must not be pierced by another tag's leader
                # (through-text), unless through-text is tolerated (spools).
        ...
        if best is not None:
            return (best[0], best[1], best[2], best[3], best[4], best[5], vert)
    return None


def _apply_move(doc, info, nx, ny, st, nb, sseg, vertical=False):
    ...
    straight leader without the expensive HasLeader off/on toggle. `vertical`
    rotates the tag 90 deg (used only when a horizontal spot did not fit)."""
    try:
        tag = info["tag"]
        ref = info["ref"]
        z = info["z"]
        try:
            tag.TagOrientation = (DB.TagOrientation.Vertical if vertical
                                  else DB.TagOrientation.Horizontal)
        except Exception:
            pass
...
def _half(info):
    """Horizontal half-dims of a tag, regardless of its current orientation --
    so searches always try horizontal first, then vertical."""
    b = info["box"]
    hw = (b[2] - b[0]) / 2.0
    hh = (b[3] - b[1]) / 2.0
    if info.get("vertical"):
        hw, hh = hh, hw     # current box is rotated; report the horizontal dims
    return hw, hh
...
def _nudge_off_obstacles(doc, view, placed_info, obstacle_boxes, tag_margin,
    ...
    def _needs_move(info):
        if _any_overlap(info["box"],
            ...
            return True
        # Overlapping another tag is always wrong (dense sheets sometimes place
        # a least-bad overlap) -- clean it up for both tools.
        for o in placed_info:
            if o is info:
                continue
            if _box_overlap(info["box"], o["box"], 0.0):
                return True
    ...
    for _ in range(4):
        ...
        for info in placed_info:
            ...
            if res is None:
                continue
            _ln, nx, ny, st, nb, sseg, vert = res
            if _apply_move(doc, info, nx, ny, st, nb, sseg, vert):
                moved = True
        if not moved:
            break
    ...
    for _ in range(3):
        ...
        for S in placed_info:
            ...
            for N in neigh[:8]:
                ...
                if sres is None:
                    continue
                _sl, sxn, syn, sst, snb, ssseg, svert = sres
                ...
                if nres is None:
                    continue
                _nl, nxn, nyn, nst, nnb, nsseg, nvert = nres
                if (_apply_move(doc, N, nxn, nyn, nst, nnb, nsseg, nvert)
                        and _apply_move(doc, S, sxn, syn, sst, snb, ssseg, svert)):
                    moved = True
                    break
        if not moved:
            break
...
def _tag_hanger_columns(doc, view, tag_symbol, hangers, obstacle_boxes,
    ...
    try:
        ...
        def _standalone_try(r, allow_obstacle=False):
            ...
            relaxed pass (`allow_obstacle=True`) tolerates a tag over a pipe.
            Tries the tag horizontal first; only if no clean horizontal spot
            exists does it try the vertical (rotated) footprint, which is narrow
            and can slot into a tight run where a wide horizontal tag cannot.
            """
            cx, cy, ebox = r["cx"], r["cy"], r["ebox"]
            check_obstacle = avoid_obstacles and not allow_obstacle
            for vert in (False, True):
                hw = (r["h"] if vert else r["w"]) / 2.0
                hh = (r["w"] if vert else r["h"]) / 2.0
                base = hh + 0.5
                    ...
                    for sgn in (1, -1):
                        ...
                        for p in placed_info:
                            ...
                            for ps in _placed_segs(p):
                                if _seg_intersect(sseg[0], sseg[1],
                                                  ps[0], ps[1]):
                                    bad = True
                                    break
                            if bad:
                        ...
                        if check_obstacle:
                            if _any_overlap(box, obstacle_boxes,
                                            obstacle_margin):
                                continue
                            if _leader_hits_obstacle([st, (hx, hy)],
                                                     obstacle_boxes, st):
                                continue
                        return (hx, hy, st, box, sseg, vert)
            return None

        def _place_standalone(r, got):
            hx, hy, st, box, sseg, vert = got
            tag = r["tag"]
            ref = r["ref"]
            z = r["z"]
            try:
                tag.TagOrientation = (DB.TagOrientation.Vertical if vert
                                      else DB.TagOrientation.Horizontal)
            except Exception:
                pass
    ...
    try:
        ...
        for cluster in clusters:
            ...
            for side, col in sides:
                ...
                # r["ebox"] is now a list of member segments; use their X extent.
                cl = min([min([min(s[0], s[2]) for s in r["ebox"]])
                          for r in col])
                cr = max([max([max(s[0], s[2]) for s in r["ebox"]])
                          for r in col])
                gap = 1.0
                best = None
                push = 0.0
            ...
            if cat == hanger_category:
                ...
                if bbox:
                    ...
                    if callout_regions and _point_in_regions(cx, cy, callout_regions):
                        in_callout_count += 1
                        continue
                    # Target geometry for the leader end: for a spool (assembly)
                    # its member pipe centrelines so the leader lands on a real
                    # pipe, not the loose assembly box; otherwise the element's
                    # box edges.
                    tgeom = (_member_segments(el, view)
                             or _box_edges(_box_of(bbox)))
                    hangers.append((el, cx, cy, z, tgeom))
...
def snap_leaders(doc, view_name=None):
    """Move each spool tag's leader end onto the spool's real member geometry.

    Fixes leaders that float off the loose assembly bounding box, without moving
    the tag head or re-laying-out the sheet -- so it is cheap and safe on any
    already-tagged view. Only tags on assemblies (spools) are touched; every
    other tag is left exactly as it is.
    """
    try:
        if view_name:
            view = next(
        ...
        tags = [tg for tg in DB.FilteredElementCollector(doc).OfClass(
            DB.IndependentTag) if tg.OwnerViewId.IntegerValue == vid]

        snapped = 0
        skipped = 0
        t = DB.Transaction(doc, "Snap Leaders to Spool")
        t.Start()
        try:
            for tg in tags:
                try:
                    refs = list(tg.GetTaggedReferences())
                    ids = list(tg.GetTaggedLocalElementIds())
                except Exception:
                    skipped += 1
                    continue
                ...
                segs = _member_segments(el, view) if el is not None else None
                if not segs:
                    skipped += 1        # not a spool/assembly -- leave it alone
                    continue
                hp = tg.TagHeadPosition
                try:
                    z = tg.GetLeaderEnd(refs[0]).Z
                except Exception:
                    z = hp.Z
                st = _leader_touch(hp.X, hp.Y, segs)
                try:
                    if tg.CanLeaderEndConditionBeAssigned(
                            DB.LeaderEndCondition.Free):
                        tg.HasLeader = True
                        tg.LeaderEndCondition = DB.LeaderEndCondition.Free
                        tg.SetLeaderEnd(refs[0], DB.XYZ(st[0], st[1], z))
                        snapped += 1
                    else:
                        skipped += 1
                except Exception:
        ...
        return {"status": "success", "view": view.Name,
                "snapped": snapped, "skipped": skipped}
    except Exception as e:
        logger.error("Error in snap_leaders: %s", e)
        return {"status": "error",
                "message": "Failed to snap leaders: {}".format(str(e))}
